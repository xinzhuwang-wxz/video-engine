...
- 🧠 **三个决策技能**(SKILL.md 开放标准,跨 agent 通用)——**纯能力,不构成固定流水线**:
  - `video-storyboard` 会写分镜(景别/运镜/旁白/节拍网格,含宣称合规闸)——最少只需要一句主旨
  - `video-previz` 会生成参考视频(Seedance,模型可换;480p 参考级省钱)——最少只需要每段一句画面描述
  - `video-editing` 会剪片(剪单→带手艺的剪映草稿:转场/滤镜/关键帧/BGM/字幕)——最少只需要素材,连意图都可以缺
  - **入口无关,按手头有什么开工**:想法先行、素材先行、AI/实拍混合都只是编排组合,由 agent 现场决定;分镜/参考片/命名规约全是"可选先验:有则增质,缺则降级自建"
