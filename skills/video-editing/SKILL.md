...
3.5 **新拟的分镜/剪辑结构必须先给用户过目**,不论花不花钱(2026-07-05 用户第二次指正:"怎么剪的分镜脚本应该让我看一下")。
    用户明确要求一步到位时也要在开剪前把结构表发出来——他可以不看,但你不能不给。
...
### 0) 盘点与归位
```bash
python3 $VE/scripts/status.py <素材库>/{SKU}/04-工作台/{排期编号}   # 状态探针:有什么/缺什么/下一步(接手先跑这个)
cat <素材库>/{SKU}/04-工作台/{排期编号}/manifest.jsonl 2>/dev/null   # 细节看索引
ls <素材目录>/                                                        # 索引缺失就扫目录(存在即状态,manifest 可重建)
```
...
- **无人值守一键出口(默认直出)**:
  `produce_cutlist.py <cutlist> --bgm <bgm> --out …_成片.mp4 --duration <目标秒数>`
  一次完成 validate → beat_align → render base → finish → manifest/report/review sheet。
- **无人值守分步出口(调试用)**:`render_cutlist.py --out …_base.mp4 --bgm <bgm>` 先出基础片,再接
  `finish_cutlist.py <cutlist> --input …_base.mp4 --out …_成片.mp4 --duration <目标秒数>`
  做成片收尾:硬字幕 PNG 叠加(当 ffmpeg 无 libass 时仍可用)、轻柔光、切点闪白、平台规格归一、抽帧 review sheet、ffprobe/音量报告。
  这是 OpenMontage 式"成片交付层"在 video-engine 的本地复用接口;保留 cutlist 可审,但不再要求 agent 为每条片子手写收尾脚本。
...
无人值守成片有 `.finish-report.json` 和 `*-review.jpg`,报告显示 9:16、30fps、音视频两条流、目标时长达标。
